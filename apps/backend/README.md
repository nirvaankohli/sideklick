...
- Put local backend secrets in repo-root `.env.backend` (copy from `.env.backend.example`). `.env` is still accepted as a temporary fallback.
- `BACKEND_JWT_SECRET` can be set in `.env.backend` (or `.env` fallback). When the Electron app starts without it, it generates and persists a local secret for that device.
