        ...
        run(...args) {
          ...
          if (normalized.startsWith("insert into gaps")) {
            ...
            tables.gaps.push({
              ...
              scope: value.scope ?? "class",
              status: "open",
              weight: value.weight,
              evidence_count: 1,
              support_signals: value.supportSignals ?? "[]",
              last_confidence: value.lastConfidence ?? null,
              last_evidence_type: value.lastEvidenceType ?? null,
              last_interaction_type: value.lastInteractionType ?? null,
          ...
          if (normalized.startsWith("insert into gap_events")) {
            ...
            tables.gap_events.push({
              ...
              evidence_type: value.evidenceType ?? null,
              support_signals: value.supportSignals ?? "[]",
              request_excerpt: value.requestExcerpt ?? null,
              created_at: nowString(),
            });
            return { changes: 1, lastInsertRowid: id };
          }
...
function buildContext() {
  return {
    ...
    workingMemory: {
      currentRequest: ["Cell respiration"],
      sessionWindow: ["Session goal: Review"],
      recentInteractions: [],
    ...
    contextTiers: {
      immediate: ["Cell respiration"],
      session: ["Session goal: Review"],
      class: [],
      historical: [],
    },
    contextPacket: {
      answering: ["Cell respiration"],
      coaching: ["Review prior confusion"],
      avoid: ["Do not let old memory override the current request."],
    },
    sessionGoal: null,
